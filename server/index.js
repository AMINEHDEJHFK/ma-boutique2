...
if (DATABASE_URL) {
    ...
    pool = new Pool({
        ...
        connectionTimeoutMillis: 5000, // Timeout de connexion plus long
        idleTimeoutMillis: 10000, // Timeout d'inactivité
...
app.get("/api/init-db", async(req, res) => {
    ...
    try {
        ...
        // On renvoie l'erreur au client pour qu'il la voie
        res.status(500).json({ error: "Erreur lors de l'initialisation", details: err.message });
    }
});

async function initializeDatabase(force = false) {
    if (!pool) throw new Error("Pool non initialisé"); // On lance une erreur si pas de pool

    const client = await pool.connect(); // On laisse l'erreur remonter si la connexion échoue
    try {
        // GRAND NETTOYAGE : On supprime la table si elle existe pour repartir de zéro
        if (force) {
            console.log("Suppression de la table products existante...");
